...
function readOptionalNonEmpty(params: URLSearchParams, key: string): QueryResult<string | null> {
  ...
  if (!normalizeString(raw)) {
    return {
      ok: false,
      error: toErrorResponse(400, "INVALID_QUERY", `Query param '${key}' cannot be empty.`, { [key]: raw }),
    };
  }
